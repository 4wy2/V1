// admin.js - نسخة احترافية آمنة (متوافقة مع كل الأجهزة + متوافقة مع WebView/CSP)
// ملاحظة: تم حذف inline onclick واستبداله بـ event delegation عشان يشتغل في بيئات غير المتصفح (WebView/Electron..)
...
async function checkUser() {
  ...
  if (teamBtn) {
    currentUser.isSuper ? teamBtn.classList.remove("hidden") : teamBtn.classList.add("hidden");
  }
  ...
  let html = `
    <div class="fixed inset-0 bg-black/90 backdrop-blur-xl z-[600] p-4 flex items-center justify-center animate-in fade-in duration-300">
      <div class="bg-slate-900 border border-white/10 w-full max-w-lg rounded-[2.5rem] p-8 relative shadow-2xl overflow-y-auto max-h-[80vh]">
        <button type="button" data-action="close-modal" class="absolute top-6 left-6 text-slate-500 hover:text-white">✕ إغلاق</button>
...
// 6. التحديث (مع إظهار الأخطاء بدل الصمت)
window.updateRowStatus = async (id, type) => {
  try {
    ...
    notify(`استثناء: ${e?.message || e}`, "error");
  }
};
...
window.updateNote = async (id, note) => {
  try {
    ...
    if (error) notify(`فشل حفظ الملاحظة: ${error.message}`, "error");
  } catch (e) {
    notify(`استثناء: ${e?.message || e}`, "error");
  }
};

// 7. الرندرة والتنسيق المتجاوب (تم إزالة inline onclick واستبداله بـ data-action/data-id)
function render() {
  ...
  const items = filtered.map((row) => {
    ...
    const typeStyle =
      { pdf: "bg-rose-500/10 text-rose-500 border-rose-500/20", png: "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" }[
    ...
    // أزرار الإجراءات الآن data-action + data-id (بدون onclick)
    if (row.status === "pending") {
      btns += `<button type="button" data-action="claim" data-id="${row.id}" class="bg-white/5 hover:bg-white/10 text-white border border-white/10 px-5 py-3 rounded-xl text-[10px] font-black transition-all">حجز</button>`;
    } else if (row.status === "reviewing" && canManage) {
      btns += `<button type="button" data-action="approved" data-id="${row.id}" class="bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-3 rounded-xl text-[10px] font-black transition-all shadow-lg shadow-emerald-600/20">اعتماد ✅</button>`;
      btns += `<button type="button" data-action="release" data-id="${row.id}" class="text-slate-500 hover:text-white px-2 py-2 text-[10px]">إلغاء</button>`;
    ...
    return {
      // Desktop: تم إصلاح td flex بوضع flex داخل div (أكثر استقراراً على Desktop/WebView)
      desktop: `<tr class="border-b border-slate-800/30 hover:bg-white/[0.01] transition-all">
        ...
        <td class="p-6 text-center">
          <span class="px-3 py-1 rounded-full text-[9px] font-black border uppercase ${typeStyle}">${row.file_type || "File"}</span>
        </td>
        <td class="p-6">
          <input type="text"
            data-note="1"
            ...
            class="w-full bg-black/40 border border-slate-800 rounded-xl p-3 text-xs text-slate-300 focus:border-blue-500 outline-none transition-all">
        </td>
        <td class="p-6 text-center text-[10px] font-black uppercase ${row.processed_by_name ? "text-blue-400" : "text-slate-600"}">
      ...
      mobile: `<div class="bg-slate-900/40 p-6 rounded-[2.5rem] border border-white/5 space-y-4 shadow-xl">
        <div class="flex justify-between items-start">
          <div>
            ...
            <p class="text-[10px] text-slate-500 font-bold mt-1 tracking-tight">👤 ${row.uploader_name || "مجهول"}</p>
          </div>
        ...
        <input type="text"
          ...
          data-id="${row.id}"
          value="${row.admin_note || ""}"
          class="w-full bg-black/60 border border-slate-800 rounded-xl p-4 text-xs text-slate-300"
          placeholder="ملاحظة الإدارة...">
        <div class="flex gap-2 justify-between pt-4 border-t border-white/5">
          ${btns}
        </div>
      </div>`,
    };
  ...
  if (desktopList) desktopList.innerHTML = items.map((i) => i.desktop).join("");
  if (mobileList) mobileList.innerHTML = items.map((i) => i.mobile).join("");
...
document.querySelectorAll(".filterBtn").forEach((btn) => {
  btn.onclick = () => {
    ...
    document.querySelectorAll(".filterBtn").forEach((b) => {
      b.className = "filterBtn flex-1 py-3 text-xs font-black rounded-xl text-slate-400 transition-all";
    });
...
// ===============================
// Event Delegation (بدون onclick)
// ===============================

// 1) أزرار الإجراءات + إغلاق مودال الفريق
document.addEventListener("click", async (e) => {
  // إغلاق مودال "شغل الشباب"
  const closeBtn = e.target.closest('button[data-action="close-modal"]');
  if (closeBtn) {
    closeBtn.parentElement?.parentElement?.remove();
    return;
  }

  // أزرار claim/approved/release
  ...
  try {
    await window.updateRowStatus(id, action);
  } finally {
    btn.disabled = false;
...
// 2) حفظ الملاحظات عند فقدان التركيز (بدون inline onblur)
document.addEventListener(
  "focusout",
  async (e) => {
    const input = e.target.closest('input[data-note="1"][data-id]');
    if (!input) return;
    const id = input.dataset.id;
    const note = input.value || "";
    await window.updateNote(id, note);
  },
  true
);
