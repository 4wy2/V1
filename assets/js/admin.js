...
const notify = (msg, type = 'info') => {
    ...
    toast.className = `${themes[type]} text-white px-6 py-4 rounded-2xl shadow-2xl font-bold text-sm mb-2 transition-all duration-500 opacity-0 transform translate-x-10`;
    toast.innerText = msg;
    container.appendChild(toast);
    setTimeout(() => { toast.classList.remove('opacity-0', 'translate-x-10'); }, 100);
    setTimeout(() => { 
...
if (loginForm) {
    loginForm.onsubmit = async (e) => {
        ...
        if (error) { notify("خطأ في البيانات", "error"); btn.disabled = false; btn.innerText = "دخول"; }
        else { checkUser(); }
    };
}
...
async function loadData() {
    ...
    if (error) return notify("مشكلة في الاتصال", "error");
    allRows = data || [];
    render();
}
...
window.updateRowStatus = async (id, type) => {
    ...
    if (!error) { notify("تم بنجاح", "success"); loadData(); }
};
...
function render() {
    ...
    const items = filtered.map(row => {
        ...
        let btns = `<a href="${row.file_url}" target="_blank" class="bg-blue-600 hover:bg-blue-500 text-white px-5 py-2.5 rounded-xl text-[10px] font-black shadow-lg shadow-blue-600/20 transition-all">فتح</a>`;
        ...
        return { desktop: `<tr class="border-b border-slate-800/30 transition-all">${rowContent}</tr>`, 
                 mobile: `<div class="bg-slate-900/40 p-6 rounded-[2.5rem] border border-white/5 space-y-4 relative">
                    ...
                    <div class="flex gap-2 justify-between pt-3 border-t border-white/5">${btns}</div></div>` };
    });
...
function updateStats() {
    ...
    if (statsDiv) statsDiv.innerHTML = `<div class="text-center"><p class="text-[8px] text-emerald-400 font-black uppercase mb-1 text-center">انجازك</p><p class="text-2xl font-black text-white">${mine.done}</p></div><div class="w-px h-8 bg-slate-800 mx-4"></div><div class="text-center"><p class="text-[8px] text-amber-400 font-black uppercase mb-1 text-center">حجزك</p><p class="text-2xl font-black text-white">${mine.pending}</p></div>`;
